...
@Injectable()
export class CreateArticle extends Command<Article> {
  public constructor(
  ...
  public internalExecute(article: Article): void {
    const articleCreated = this.shoppingCartRepository.createArticle(article)
    this.stateManager.state.shoppingCart.getArticles().push(articleCreated)
  }
}
